  ...
  return (
    <div className="flex flex-col gap-2 py-2 border-b last:border-b-0" style={{ borderColor: COLORS.LINE }}>
      ...
      <div className="flex flex-col gap-1.5">
        {options.map((o, i) => {
          const isSelected = interactive ? selectedArr.indexOf(i) !== -1 : o.selected;
          return (
            <button
              key={o.text}
              type="button"
...
function Pattern({ id, n, label, together, togetherLabel, soWhat, chips, chipDetails, reflect, flow, systems, accent, crossing, internalSynthesis, isOpen, onToggleOpen }) {
    ...
    <div id={id} className="flex flex-col gap-3" style={{ scrollMarginTop: "80px" }}>
      <button onClick={() => setOpened(!opened)} aria-expanded={opened} className="w-full text-left flex flex-col gap-1.5">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="uppercase flex-shrink-0 mt-1" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", letterSpacing: "0.06em", color: COLORS.FAINT }}>{opened ? "Hide ▲" : "Read ▼"}</span>
        </div>
        {crossing && (
...
function ClaimCard({ claim }) {
  ...
  return (
    <div className="rounded-lg p-5 flex flex-col gap-3" style={{ background: COLORS.CARD, boxShadow: SOFT_SHADOW, borderLeft: `4px solid ${rel.color}` }}>
      <div className="flex items-start justify-between gap-3">
        <span className="uppercase" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "9.5px", letterSpacing: "0.1em", color: rel.color, fontWeight: 700 }}>
          {rel.label}
        </span>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {claim.evidence.length > 2 && (
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "9.5px", color: COLORS.FAINT, paddingLeft: "2px" }}>
            +{claim.evidence.length - 2} more, tap below
          </p>
        )}
      </div>
                ...
                <div className="mt-2 pt-4 flex flex-col gap-5" style={{ borderTop: `1px solid ${COLORS.LINE}` }}>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {crossCheck.relationship === "lineUp" && (
                      <p className="pl-1" style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", lineHeight: 1.55, color: COLORS.MUTED, fontStyle: "italic" }}>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="pl-1" style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", lineHeight: 1.55, color: COLORS.MUTED }}>
                    ...
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10.5px", color: COLORS.RED, paddingLeft: "4px" }}>See Synthesis, Discovery 1, for what this specific tension means.</span>
                  </div>
                </div>
              )}
            </div>
  ...
  return (
          <div className="flex flex-col gap-8">
            ...
            <Pattern
              ...
              together={`${VOICE.purpose} Saturn's slow-built credibility and a ${humanDesign.type} design that only moves on its own terms aren't two separate demands on your career. That doesn't just mean you're independent at work, it suggests authority here was never available to borrow, it had to be built inside terrain you chose yourself, which is exactly why the years that felt slow were never actually wasted.`}
            ...
            <div className="rounded-lg p-5 sm:p-6 flex flex-col gap-3" style={{ background: COLORS.CARD, boxShadow: SOFT_SHADOW, borderLeft: `4px solid ${COLORS.RED}` }}>
              ...
              {claims[0] && (
                <div>
                  ...
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "13px", lineHeight: 1.6, color: "#3A362C" }}>
                    {claims[0].claim} This one has the most independent support of anything in your report, {claims[0].systems.length} systems. Stop treating it as a preference you should moderate and start treating it as the material you build with.
                  </p>
                </div>
              )}
              {tensions[0] ? (
              ...
              {vedic.isReal && (
                <div>
                  ...
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "13px", lineHeight: 1.6, color: "#3A362C" }}>
                    Your {vedic.mahaGraha}/{vedic.bhuktiGraha} period closes around {vedic.closingLabel}. Whatever you've been trying to force a conclusion on has an actual end date, and it isn't today. Finish what belongs to now. Let the next chapter start what belongs to it.
                  </p>
                </div>
              )}
              <div>
            ...
            <details className="rounded-lg p-4" style={{ background: "#F2ECDD", border: `1px solid ${COLORS.LINE}` }}>
              <summary className="cursor-pointer list-none flex items-center gap-2" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", color: COLORS.MUTED }}>
                <span style={{ color: COLORS.GOLD }}>+</span> This report updates as your timing shifts, and what's next
              </summary>
