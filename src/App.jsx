...
const utHours = utHoursOverride !== null ? utHoursOverride : timeStr ? (() => {
...
const snapped = Math.abs(diff - Math.round(diff / 30) * 30) < CUSP_EPSILON ? Math.round(diff / 30) * 30 : diff;
...
const framing = soul ? `${subject} shows the soul-level version of what the Tropical ${planet} shows on the surface: ${core.replace(/^shows /, "")}.` : `${subject} ${core}.`;
...
if (p.humanDesign.basis === "computed" && (p.humanDesign.type === "Generator" || p.humanDesign.type === "Manifesting Generator" || p.humanDesign.type === "Projector")) {
...
if (p.humanDesign.basis === "computed" && (p.humanDesign.type === "Manifestor" || p.humanDesign.type === "Projector")) {
...
if (p.humanDesign.basis === "computed" && (p.humanDesign.type === "Manifesting Generator" || p.humanDesign.type === "Manifestor")) {
...
detect: (p, h) => {
const out = [];
if (p.humanDesign.basis === "computed" && (p.humanDesign.type === "Generator" || p.humanDesign.type === "Manifesting Generator")) {
...
const confidenceTier = strength >= 4 ? { label: "High Confidence", color: "#3F7D5C" } : strength === 3 ? { label: "Moderate Confidence", color: COLORS.GOLD } : { label: "Lower Confidence", color: COLORS.RED };
results.push({
id: c.id,
claim: c.claim,
meaning: c.meaning,
...
const confidence = passCount >= 3 ? "High, most independent placements agree" : passCount === 2 ? "Moderate, two placements agree" : passCount === 1 ? "Mixed, only one placement agrees" : "Low, none of the checked placements agree";
...
{humanDesign.basis === "computed" ? ` and ${humanDesign.type} design` : ""} all point toward the same behavior:
...
id="q-hd"
title="Energy Type"
source={humanDesign.basis === "computed" ? "computed" : humanDesign.basis === "known" ? "self-report" : humanDesign.basis === "proxy" ? "self-report" : "generated"}
explainer={humanDesign.basis === "computed"
...
const careerFlowItems = humanDesign.basis === "computed"
? [
...
const careerTogether = humanDesign.basis === "computed"
...
const careerChipDetails = humanDesign.basis === "computed"
? [
...
{ label: `HD: ${humanDesign.type}, ${humanDesign.authority}`, system: "Energy Type (Human Design)", calculation: "Swiss Ephemeris positions mapped through 64 gates", meaning: "Type and authority together, computed for real." },
]
: [
{ label: `Saturn: ${houseOf(tp.Saturn)} house`, system: "Identity (Tropical)", calculation: "Saturn's longitude mapped to the selected house system", meaning: "Career and public authority placement." },
{ label: "Draconic NN near Saturn", system: "Soul Layer (Draconic)", calculation: "Chart shifted so North Node sits at 0° Aries", meaning: "A soul-level marker echoing the same career point." },
...
Enneagram describes recurring motivational and behavioral patterns. Astrology describes symbolic patterns
from your birth data. This doesn't assume they're the same thing, it measures whether,
